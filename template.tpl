...

{
  ...
  "description": "Bekijk je client side requests",
  "containerContexts": [
    "WEB"
  ],
  "securityGroups": []
...
[
  {
    ...
    "help": "Zoek de Container Identifier op in je Taggrs account",
    "valueValidators": [
      {
        "type": "STRING_LENGTH",
        "args": [
...
const transformData = () => {
  ...
  const gcsValue = 'G1' + (adStorageGranted ? '1' : '0') + (analyticsStorageGranted ? '1' : '0');
  
  const adStorageLetter = adStorageGranted ? 'r' : 'p';
  const analyticsStorageLetter = analyticsStorageGranted ? 'r' : 'p';
  const adUserDataLetter = adUserDataGranted ? 'r' : 'p';
  const adPersonalizationLetter = adPersonalizationGranted ? 'r' : 'p';
  
  const gcdValue = '11' + adStorageLetter + '1' + analyticsStorageLetter + '1' + adUserDataLetter + '1' + adPersonalizationLetter + '5';
  ...
  const parameters = [
    ...
    {key: 'gcs', value: gcsValue},
    {key: 'gcd', value: gcdValue}
  ];
...
- name: Untitled test 1
  code: |-
    const mockData = {
      domainName: 'https://api.extension.taggrs.io/api/test?container_id=1234567890&gtmTagId=2147483646&gtmEventId=1'
    };
